...
// Platform logo components - memoized for performance
const RedditLogo = memo(() => (
...
const TwitterLogo = memo(() => (
  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
));

const LinkedInLogo = memo(() => (
  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
...
export const HomePage = memo(() => {
  ...
  const { engagementRate, postsThisWeek, scheduledPosts } = useMemo(() => {
    if (!analytics) {
      return {
        engagementRate: 0,
        postsThisWeek: 0,
    ...
    return {
      engagementRate: analytics.analytics.length > 0
        ? (analytics.analytics.reduce((acc, curr) => acc + curr.likes + curr.comments + curr.shares, 0) / analytics.analytics.length) / 100
        : 0,
      postsThisWeek: analytics.posts.filter(p => new Date(p.published_at) > weekAgo).length,
      scheduledPosts: analytics.posts.filter(p => p.status === 'scheduled').length
    };
  }, [analytics]);
  ...
  ], [engagementRate, postsThisWeek, scheduledPosts]);
